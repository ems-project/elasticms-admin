...
use App\Import\Chamber\XML\Members;
...
class Import
{
    ...
    public function __construct(Client $client, LoggerInterface $logger, string $dir, string $type, string $environment, bool $dryPdf)
    {
        $this->client = $client;
        $this->logger = $logger;
        $this->dryPdf = $dryPdf;
    ...
    public function search(array $body): array
    {
        return $this->client->search([
            'index' => 'webchamber_ma_'.$this->environment,
            'type' => 'doc',
            'body' => $body,
        ]);
    }
    ...
    public function getLegislature(int $id): array
    {
        if (!$this->existLegislature($id)) {
            throw new \Exception(sprintf('Legislatue unknown %s', $id));
        }
    ...
    private function buildLegislatures(): array
    {
        ...
        $result = $this->client->search([
            'index' => 'webchamber_'.$this->environment,
            'type' => 'legislature',
            'size' => 100,
