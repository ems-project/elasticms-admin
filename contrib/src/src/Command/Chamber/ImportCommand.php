...
class ImportCommand extends Command implements CommandInterface
{
    ...
    {
        list($dir, $type, $importId, $environment, $dry, $clean, $limit, $bulkSize, $dryPdf) = $this->getOptions($input);
        ...
        $import = new Import($this->client, $logger, $dir, $type, $environment, $dryPdf);
        $factory = new ModelFactory($indexHelper, $import, $this->storageManager, $this->extractorService, $this->fileService);

        foreach ($this->findFiles($style, $dir, $type) as $file) {
            ...
            if ($model && !$dry) {
                $this->bulker->indexDocument($model, $indexHelper->getIndex($model));
            }
        }
        ...
        foreach ($factory->getChildren() as list($childType, $children)) {
            ...
            if (!$dry) {
                ...
                foreach ($children as $child) {
                    $this->bulker->indexDocument($child, $indexHelper->getIndex($child), true);
    ...
    private function findFiles(SymfonyStyle $style, string $dir, string $type): \Generator
    {
